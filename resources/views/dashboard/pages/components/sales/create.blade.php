...
<head>
    ...
    <style>
        ...
        /* Modernized Payment Error Popup */
        #payment-error-popup {
        ...
        #payment-error-popup button {
            transition: background-color 0.2s ease;
        }

        #payment-error-popup button:hover {
...
<div class="max-w-7xl mx-auto bg-white p-8 rounded-lg shadow-md">
    <div class="flex justify-between items-center mb-8">
        ...
        @php
            $dashboardRoute = auth()->user()->isEmployee() ? route('employee.dashboard')
                            : (auth()->user()->isAdmin() ? route('sales.index')
                            : (auth()->user()->isSuperAdmin() ? route('sales.index') : '#'));
        @endphp
    ...
    <form action="{{route('sales.store')}}" method="POST" onsubmit="return validateForm()">
        ...
        <div class="grid grid-cols-1 md:grid-cols-6 gap-4 mb-6">
            ...
            <div>
                <label for="receivable_amount" class="block text-sm font-medium text-gray-700 mb-1">Receivable Amount</label>
                <input type="number" id="receivable_amount" name="receivable_amount" step="0.01" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" readonly>
            </div>
            <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-1 gap-4 mb-6">
            <div>
                ...
                <input type="number" id="total_payment_amount" step="0.01" class="w-full px-3 py-2 border border-gray-300 rounded-md focusSAFE:outline-none focus:ring-2 focus:ring-blue-500" readonly>
            </div>
        </div>
    ...
    <!-- Modernized Payment Error Popup -->
    <div id="payment-error-popup" class="hidden fixed bottom-6 right-6 text-white p-5 rounded-lg z-50">
        <div class="flex items-start space-x-3">
            <svg class="w-6 h-6 flex-shrink-0 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
            </svg>
            <div>
                <p>The total payment amount does not match the net total amount. Please enter the correct amount.</p>
                <button onclick="closePaymentErrorPopup()" class="mt-3 inline-block px-3 py-1 rounded-full bg-white bg-opacity-20 hover:bg-opacity-30 transition">Close</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function updatePaymentFields() {
        ...
        const receivableInput = document.getElementById('receivable_amount');
        ...
        if (paymentOption === 'Cash') {
            ...
            cardInput.value = '';
            creditInput.value = '';
            receivableInput.value = netTotalAmount.toFixed(2);
    ...
    function updateTotalPayment() {
        ...
        // Update receivable amount for 'Other' payment option
        if (paymentOption === 'Other') {
            receivableInput.value = nonCreditTotal.toFixed(2);
        }
        ...
        // Check if total payment matches net total amount
        if (Math.abs(totalPayment - netTotalAmount) > 0.01) {
    ...
    $(document).ready(function() {
        const amountInputs = ['rate_input', 'tax_percentage_input', 'discount', 'cash_amount', 'upi_amount', 'card_amount', 'credit_amount', 'receivable_amount'];
    ...
    function loadBillDetails(billId) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#receivable_amount').val(master.receivable_amount || '');
